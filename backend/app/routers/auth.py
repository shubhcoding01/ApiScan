...
def register_user(user: UserCreate, db: Session = Depends(get_db)):
    """
    Creates a new user in the real database.
    """
    # Prevent users from registering the hardcoded test email
    if user.email in FAKE_USER_DB:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Reserved email address"
        )
...
# ---------------------------------------------------------
# 2. LOGIN (Hybrid: Fake DB + Real DB)
# ---------------------------------------------------------
@router.post("/login")
def login(
    form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    """
    Authenticates a user. Checks the FAKE_USER_DB first for testing, 
    then falls back to the real database.
    """
    # --- 1. Check Fake Test DB First ---
    fake_user = FAKE_USER_DB.get(form_data.username)
    if fake_user:
        if not verify_password(form_data.password, fake_user["hashed_password"]):
            raise HTTPException(status_code=401, detail="Invalid email or password")
            
        access_token = create_access_token(
            data={"sub": fake_user["id"]},
            expires_delta=timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)
        )
...
def get_current_user(
    ...
    try:
        ...
        # --- 2a. Check if it's the Fake Test User ---
        if user_id == FAKE_USER_DB["admin@apiscan.com"]["id"]:
            # We return a dictionary here; FastAPI will automatically cast it to UserResponse
            return {
                "id": user_id,
                "email": "admin@apiscan.com",
                "full_name": FAKE_USER_DB["admin@apiscan.com"]["full_name"],
                "is_active": FAKE_USER_DB["admin@apiscan.com"]["is_active"],
                "created_at": datetime.utcnow()
            }

        # --- 2b. Fetch user from real DB ---
        user = db.query(User).filter(User.id == user_id).first()
