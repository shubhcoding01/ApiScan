...
def add_secret(
    ...
    # --- FIX: Handle 'default' environment error ---
    # The DB expects 'DEV', 'STAGING', or 'PROD'. 
    # If 'default' comes in, we map it to 'DEV'.
    valid_environment = payload.environment
    if valid_environment == "default":
        valid_environment = "DEV"
    # -----------------------------------------------
    ...
    secret = SecretVault(
        ...
        key_name=payload.key_name,
        encrypted_value=encrypted_value,
        environment=valid_environment,  # <--- Use the fixed value here
    )
