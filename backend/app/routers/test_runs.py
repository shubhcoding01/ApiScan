...
# Helper to handle both DB User objects and Fake User dicts safely
...
def create_test_run(
    ...
    # 🔒 SECURITY: Verify the blueprint exists AND the user owns the parent project
    blueprint = (
        db.query(TestBlueprint)
        .join(Project, TestBlueprint.project_id == Project.id)
        .filter(
            TestBlueprint.id == payload.test_blueprint_id,
    ...
    if not blueprint:
        raise HTTPException(404, "Test blueprint not found or access denied")
    ...
    logger.info(f"Test run {test_run.id} started by user {user_id}")
...
def list_test_runs_for_blueprint(
    ...
    # 🔒 SECURITY: Only return runs if the user owns the parent project
    return (
        db.query(TestRun)
        .join(TestBlueprint, TestRun.blueprint_id == TestBlueprint.id)
        .join(Project, TestBlueprint.project_id == Project.id)
        .filter(
            TestRun.blueprint_id == blueprint_id,
            Project.user_id == user_id
        )
        .order_by(TestRun.created_at.desc())
        .all()
...
def get_test_run(
    ...
    # 🔒 SECURITY: Ensure the user owns the project this test belongs to
    test_run = (
        db.query(TestRun)
        .join(TestBlueprint, TestRun.blueprint_id == TestBlueprint.id)
        .join(Project, TestBlueprint.project_id == Project.id)
    ...
    if not test_run:
        raise HTTPException(404, "Test run not found or access denied")

    return test_run


# ---------------------------------------------------------
# WORKER COMPLETION CALLBACK (Internal Use)
# ---------------------------------------------------------
@router.post("/{test_run_id}/complete")
def complete_test_run(
    test_run_id: UUID,
    ...
    db: Session = Depends(get_db)
    # ⚠️ Note: We DO NOT require get_current_user here because this endpoint 
    # is called by your automated background worker, not the human user.
):
    """
    Called by the Celery Worker when testing is finished.
    Updates the Run status to 'PASSED' or 'FAILED'.
    """
