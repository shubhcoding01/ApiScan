...
export default function TestRunsListPage() {
  ...
  // --- STATE ---
  const [runs, setRuns] = useState<TestRun[]>([]);
  // Store blueprint names for better UX (ID -> Name)
  const [blueprintsMap, setBlueprintsMap] = useState<Record<string, string>>({}); 
  const [isLoading, setIsLoading] = useState(true);
  const [startingRunId, setStartingRunId] = useState<string | null>(null);
  ...
  const fetchRuns = useCallback(async () => {
    try {
      ...
      // 2. Get Blueprints (to link IDs to Names)
      const blueprintRequests = versions.map((v: any) =>
        api.get(`/test-blueprints/api-version/${v.id}`)
      );
      const blueprintResults = await Promise.allSettled(blueprintRequests);
      ...
      blueprints.forEach(bp => {
        // Try to find a readable name in the strategy JSON, fallback to generic
  ...
  const handleRunAgain = async (blueprintId: string) => {
    setStartingRunId(blueprintId);
    try {
      // Assuming endpoint is POST /test-blueprints/{id}/run
      // Adjust if your backend uses POST /test-runs/ with body
      const { data } = await api.post(`/test-blueprints/${blueprintId}/run`);
      
      // Handle response ID variation
      ...
      console.error('Failed to start test run', err);
      // Fallback method if previous endpoint fails
      try {
         const { data } = await api.post('/test-runs/', { blueprint_id: blueprintId });
         router.push(`/projects/${projectId}/test-runs/${data.id}`);
      } catch (retryErr) {
         alert('Failed to start test run.');
      }
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans selection:bg-blue-500/30">
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56 bg-[#0B1120] border-slate-800 text-slate-300" align="end">
                  ...
                  <DropdownMenuItem className="hover:bg-slate-800 cursor-pointer text-red-400">Log out</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="flex-1 mb-0 pb-0">
            <PageHeader
        ...
        <div className="flex items-center gap-4">
            <div className="relative flex-1 max-w-md">
                ...
                <Input 
                    placeholder="Search by ID, Status, or Strategy..." 
                    className="pl-9 bg-slate-900/50 border-slate-800 focus:ring-blue-500/50 text-slate-200 placeholder:text-slate-600"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                />
            </div>
            <Button variant="outline" className="border-slate-800 bg-slate-900/50 text-slate-400 hover:text-white hover:bg-slate-800">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 border border-dashed border-slate-800 rounded-2xl bg-slate-900/20 text-center">
            <div className="w-16 h-16 bg-slate-900 rounded-full flex items-center justify-center mb-6 border border-slate-800">
                <LayoutList className="w-8 h-8 text-slate-600" />
            </div>
          ...
          <Card className="border-slate-800 bg-slate-900/30 backdrop-blur-sm overflow-hidden shadow-xl">
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-950/50 border-b border-slate-800 text-slate-500 font-medium uppercase tracking-wider text-xs">
                  <tr>
                    <th className="px-6 py-4">Status</th>
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {filteredRuns.map((run) => {
                    ...
                    return (
                        <tr key={run.id} className="group hover:bg-slate-800/30 transition-colors">
                            <td className="px-6 py-4">
                                <TestRunStatus status={run.status} />
                            </td>
                            ...
                            <td className="px-6 py-4">
                                <Badge variant="outline" className="font-mono text-xs text-slate-500 border-slate-800 bg-slate-950">
                                    #{run.id.slice(0, 8)}
                                </Badge>
                            </td>
                            ...
                            <td className="px-6 py-4 text-right">
                                <div className="flex justify-end items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <Link href={`/projects/${projectId}/test-runs/${run.id}`}>
                                        <Button variant="ghost" size="sm" className="h-8 text-xs hover:bg-slate-800 text-slate-300">
                                            View Logs
                                            <ArrowRight className="w-3 h-3 ml-2" />
                                        </Button>
                                    </Link>
...
function RunsSkeleton() {
    return (
        <div className="space-y-4">
            ...
            <Skeleton className="h-20 w-full bg-slate-900/50 rounded-lg" />
            <Skeleton className="h-20 w-full bg-slate-900/50 rounded-lg" />
            <Skeleton className="h-20 w-full bg-slate-900/50 rounded-lg" />
        </div>
    );
