...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formData = new URLSearchParams();
      formData.append('username', email.trim()); // email = username
      formData.append('password', password);
    ...
    <div className="w-full max-w-md p-8 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl">
      <div className="text-center mb-8">
        ...
        <p className="text-zinc-400 text-sm">
          Sign in to access your autonomous QA agent
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setEmail(e.target.value)}
              placeholder="admin@apiscan.com"
              className="w-full pl-10 pr-4 py-2 bg-black border border-zinc-800 rounded-lg text-white focus:ring-2 focus:ring-blue-600"
            />
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex justify-center items-center"
        >
          {isLoading ? (
            <>
              <Loader2 className="w-4 h-4 mr-2 animate-spin" />
