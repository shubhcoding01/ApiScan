...
# 👈 FIX: Changed localhost to 'redis' for Docker compatibility fallback
...
celery_app = Celery(
    ...
    broker=REDIS_URL,
    backend=REDIS_URL,
    # 👈 FIX: Point to the Python file (module), not the individual functions inside it
...
# QUEUE ROUTING
# ----------------------------------------------------
# 👈 FIX: Use the full module path to perfectly match how Celery registers them
celery_app.conf.task_routes = {
    "worker.app.tasks.execute_test_run": {"queue": "test_execution"},
    "worker.app.tasks.generate_test_blueprint": {"queue": "ai_generation"},
    "worker.app.tasks.ingest_spec": {"queue": "spec_ingestion"},
    "worker.app.tasks.detect_breaking_changes": {"queue": "diff_analysis"},
}